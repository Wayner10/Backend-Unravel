  ...
  try {
    const result = await client.query(
      `INSERT INTO tb_places (
          place_name, place_desc, place_score, place_price_adult,
          place_price_children, place_lat, place_lng, place_canton,
          place_nearest_city, place_waze_url, place_google_url,
          place_website_url, place_open_time, place_close_time,
...
// Route to update a places
router.put("/places/:place_id", async (req, res) => {
  const place_id = parseInt(req.params.place_id);
  const { place_name, place_desc, place_score } = req.body;

  try {
    const result = await client.query(
      "UPDATE tb_places SET place_name = $1, place_desc = $2, place_score = $3 WHERE place_id = $4 RETURNING *",
      [place_name, place_desc, place_score, place_id]
    );
